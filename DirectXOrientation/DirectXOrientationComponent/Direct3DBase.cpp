...
// Initialiser les ressources Direct3D devant être exécutées.
...
// Il s'agit des ressources dépendantes du périphérique.
...
void Direct3DBase::UpdateDevice(_In_ ID3D11Device1* device, _In_ ID3D11DeviceContext1* context, _In_ ID3D11RenderTargetView* renderTargetView)
{
	...
	if (m_d3dDevice.Get() != device)
	{
		...
		// Forcer l'appel de CreateWindowSizeDependentResources.
	...
	// Mettez en cache les dimensions de la cible de rendu dans notre classe d'assistance pour en faciliter l'utilisation.
	...
	// Définir la fenêtre d'affichage de rendu de manière à ce qu'elle cible la totalité de la fenêtre.
...
// Alloue toutes les ressources de mémoire dépendant de la taille de la fenêtre.
void Direct3DBase::CreateWindowSizeDependentResources()
{
	// Créer un affichage gabarit.
